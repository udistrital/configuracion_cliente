...
angular.module('configuracionApp')
  .controller('menuCtrl', function($location, $http, $window, $q, $scope, $rootScope, token_service, configuracionRequest, notificacion, $translate, $route, $mdSidenav) {
    ...
    $scope.perfil = "Administrador configuración";
    ...
    if(token_service.live_token()){
        ...
        if (!angular.isUndefined($scope.token.role)){
            ...
            //configuracionRequest.get('menu_opcion_padre/ArbolMenus/' + roles + '/Configuración','').then(function(response) {
            configuracionRequest.get('menu_opcion_padre/ArbolMenus/' + $scope.perfil + '/Configuración','').then(function(response) {
